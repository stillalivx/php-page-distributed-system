...
<html lang="es">
    <head>
        <title>Municipio | Recursos Humanos</title>
    ...
    <body class="bg-neutral-900">
        <div class="grid grid-rows-[auto_1fr] my-5">
            <header class="grid grid-cols-[15%_1fr]">
                <div></div>
                <h1 class="text-4xl font-medium text-slate-200 ml-5">Municipio</h1>
            </header>
                        ...
                        <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" class="grid grid-cols-[1fr_auto] items-center mb-0">
                            <div class="mr-10">
                                <div class="grid grid-rows-[auto_auto]">
                                    ...
                                    <input required type="text" name="name" id="name" placeholder="Nombre del municipio" value="<?php echo isset($_GET["name"]) ? $_GET["name"] : "" ?>" class="outline-none bg-neutral-800 border-solid border-b-2 py-1 border-neutral-700 text-neutral-50 focus:border-blue-400" />
                                </div>
                                <div class="hidden">
                                    <label for="id"></label>
                                ...
                                <table class="w-full">
                                    ...
                                    <?php foreach ($result as $key => $value): ?>
                                        <tr class="text-white">
                                            ...
                                            <td class="border border-solid border-neutral-500 py-2.5 px-3 w-full"><?php echo $value["NOMBRE"]; ?></td>
                                            <td class="border border-solid border-neutral-500 py-2.5 px-3">
                                                <div class="flex text-xl">
                                                    <button class="mr-5" onclick="updateItem('estado', '<?php echo "action=update&id={$value["ID"]}&name={$value["NOMBRE"]}"; ?>')">
                                                        <i class="bx bx-edit-alt text-amber-300"></i>
                                                    </button>
                                                    <button onclick="deleteItem('estado', <?php echo $value["ID"]; ?>)">
